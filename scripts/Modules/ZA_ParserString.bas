...
Public Sub ParserStringWB()
    ...
    If Not objWB.FullName Like "*" & Application.PathSeparator & "*" Then
        Call MsgBox("Selected file [" & sNameWB & "] not saved, save the file to continue!", vbCritical, "Error:")
        Exit Sub
...
Private Sub ParserStr(ByRef WBString As Workbook, ByRef WBNew As Workbook)
    ...
    Application.Calculation = xlCalculationAutomatic
    Application.ScreenUpdating = True
    Call MsgBox("The string data of the book [" & sNameFile & "] is collected!", vbInformation, "Data collection:")
...
Private Sub ParserStrForms(ByRef WB As Workbook, ByRef WBNew As Workbook)
    ...
    With WBNew.Worksheets(SH_NAME_FORM)
        ...
        .Cells(1, 2).Value = "TYPE FORM/CONTROL SYSTEM"
    ...
    If (Not Not arrStr) <> 0 Then
        ...
        End With
        Debug.Print "Completed-collecting UserForms control rows"
    Else
        Debug.Print "Completed-collecting UserForms strings, UserForms is not in the file"
    End If
End Sub
...
Private Sub ParserStrUI(ByRef WB As Workbook, ByRef WBNew As Workbook, Optional bRenameUI As Boolean = False)
    ...
    If VBA.UCase$(WB.Name) Like "*.XLS" Then
        Debug.Print "UI string collection is not possible in files with the extension [*. xls]"
        Debug.Print "Resave the file to the new format"
        Exit Sub
    End If
    ...
    With cEditOpenXML
        ...
        If FileHave(sFullNameXML & "customUI.xml") Then
            If Not bRenameUI Then
                Debug.Print "Start-collecting UI strings ribbon panel UI"
                Call ParserStrUIMain(WBNew, SH_STRING & "UI", sFullNameXML & "customUI.xml")
                Debug.Print "Completed-collecting rows of the ribbon panel UI"
            Else
                Debug.Print "Start-renaming the rows of the ribbon panel UI"
                Call ReNameStrUI(WBNew, SH_STRING & "UI", sFullNameXML & "customUI.xml")
                Debug.Print "Completed-renaming the rows of the ribbon panel UI"
            End If
        Else
            Debug.Print "customUI Ribbon Panel - No"
        End If
        If FileHave(sFullNameXML & "customUI14.xml") Then
            If Not bRenameUI Then
                Debug.Print "Start-collecting UI strings ribbon panel UI14"
            ...
            Else
                Debug.Print "Start-renaming rows of ribbon panel UI14"
            ...
            End If
        Else
            Debug.Print "Ribbon panel customUI14-no"
        End If
        .ZipAllFilesInFolder
    End With
...
Public Sub ReNameStr()
    If MsgBox("Continue executing [Rename String values] ?" & vbNewLine & "This operation cannot be canceled!", vbYesNo + vbQuestion, "Renaming rows:") = vbNo Then
        Exit Sub
    End If
    ...
    With ActiveSheet
        If .Name = SH_NAME_SET Then
            ...
            If FileHave(sPath) Then
                ...
                End If
            Else
                Call MsgBox("File not found on the sheet: [" & SH_NAME_SET & "]", vbCritical, "Error:")
            End If
        Else
            Call MsgBox("Create or navigate to a sheet: [" & SH_NAME_SET & "]", vbCritical, "Search for settings:")
        End If
    End With
End Sub
...
Private Sub ReNameFormControls(ByRef WB As Workbook, ByRef WBNew As Workbook)
    ...
    For i = 1 To UBound(arrData)
        If CheckVBComponent(objVB, arrData(i, 1)) Then
            ...
            If arrData(i, 2) = "FORMA" Then
                If arrData(i, 8) <> vbNullString Then Call SetPropertisForm(objVBCom, arrData(i, 8))
            Else
                If CheckControlOnForm(objVBCom, arrData(i, 3)) Then
                    ...
                    End With
                Else
                    arrData(i, 10) = "Controller not found"
                End If
            End If
        Else
...
Private Sub ParserStringsInCodeAdd(ByRef WB As Workbook, ByRef WBNew As Workbook)
    ...
    Debug.Print "Start - collecting rows in modules"
